...
import { getAcaoConteudo, adminUpdateAcaoConteudo } from "@/lib/api";
...
export default function AdminConstrutorAcaoPage() {
  ...
  const idAcao = params.id as string; // Pega o ID da ação lá da URL (ex: /acao/3)
  ...
  // 1. CARREGAR OS BLOCOS DO BANCO DE DADOS
  useEffect(() => {
    const carregarConteudo = async () => {
      try {
        const dados = await getAcaoConteudo(idAcao);

        // Se já existirem blocos no banco, carrega eles. Se não, começa com um bloco vazio padrão.
        ...
        // Fallback para não quebrar a tela caso dê erro
        setBlocks([
          {
            id: Date.now().toString(),
            type: "text",
  ...
  const handleSalvar = async () => {
    ...
    setIsSaving(true);
    try {
      await adminUpdateAcaoConteudo(idAcao, blocks, token);
      alert("Página salva com sucesso!"); // Opcional: Você pode trocar por um Toast bonito depois
    } catch (error) {
      console.error(error);
      alert("Erro ao salvar a página. Tente novamente.");
    } finally {
      setIsSaving(false);
    }
  };

  // Funções de manipulação de blocos (Mantidas iguais ao seu código)
  const addTextBlock = () =>
    setBlocks([
      ...blocks,
      {
